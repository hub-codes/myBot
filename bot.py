...
# اتصال به Redis (در صورت فراهم بودن)
r = None
if REDIS_URL and (REDIS_URL.startswith("redis://") or REDIS_URL.startswith("rediss://")):
    ...
    logger.info("ℹ️ No Redis URL provided, statistics disabled.")
...
async def send_question(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id):
    """ارسال سوال جاری و تنظیم تایمر"""
    data = user_quiz.get(user_id)
    if not data or data["current_index"] >= len(data["questions"]):
        return
    q = data["questions"][data["current_index"]]
    # کلید callback شامل اندیس سوال برای جلوگیری از پاسخ به سوالات منقضی
    keyboard = [[InlineKeyboardButton(opt, callback_data=f"ans:{data['current_index']}:{i}")] for i, opt in enumerate(q["options"])]
    reply_markup = InlineKeyboardMarkup(keyboard)
...
async def timeout_handler(context: ContextTypes.DEFAULT_TYPE):
    ...
    # حذف دکمه‌ها و نمایش پیام تایم‌اوت
    try:
...
async def answer_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """پردازش پاسخ کاربر (بدون بازخورد وسط آزمون)"""
    query = update.callback_query
    await query.answer()
    user_id = update.effective_user.id
    quiz = user_quiz.get(user_id)
    ...
    # استخراج اندیس سوال و گزینه از callback data
    ...
    # بررسی اینکه پاسخ برای سوال جاری است (نه سوال منقضی‌شده)
    if q_index != quiz["current_index"]:
        await query.answer("این سوال منقضی شده است.", show_alert=False)
        return
...
async def finish_quiz(context, user_id, chat_id):
    ...
    # ذخیره آمار در Redis (در صورت وجود)
    if r:
        try:
...
async def stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_id = update.effective_user.id
    if not r:
        await update.message.reply_text("📭 سیستم آمار موقتاً در دسترس نیست (Redis تنظیم نشده).")
        return
    try:
        key = f"quizstats:{user_id}"
        data = r.hgetall(key)
...
def main():
    ...
    if use_webhook:
        ...
        logger.info(f"Running webhook on port {PORT}, URL: {WEBHOOK_URL}")
        app.run_webhook(
            listen="0.0.0.0",
            port=PORT,
        ...
        logger.info("Running polling... (Local test)")
